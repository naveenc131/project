...
def closer_city(lat, lon, city1, city2):
    """ Returns the name of either city1 or city2, whichever is closest
        ...
        'Bucharest'
    """
    city = make_city('abc',lat,lon)
    distance1 = (city1,city)
    distance2 = (city2,city)
    if distance1 > distance2:
        return getname(city2)
    return getname(city1)
...
def create_board(rows, columns):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [create_row(column)for element in range(rows)]
...
def put_piece(board, max_rows, column, player ):
    ...
    -1
    """
    curr_index = max_rows -1
    while curr_index <= 0 and get_piece(board,curr_index,column) != '-':
        if curr_index >= 0:
            new_row = replace_elem(board[curr_index],curr_index,columns)
            new_board = replace_elem(board, curr_row_index, new_row)
            board = new_board
        return curr_index,board
